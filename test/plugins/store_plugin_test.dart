...
void main() {
  ...
  test('it should store and retrieve a value on a request', () async {
    ...
    app.all('/test', (req, res) {
      expect(req.methodRoute, '/test');
      req.keyValueStore.set('testValue', 'bah!');
      expect(req.keyValueStore.get<String>('testValue'), 'bah!');
      didHit = true;
      return 'done';
    });
  ...
  test('the store is correctly available across multiple routes', () async {
    var didHit = false;
    app.get('*', (req, res) {
      req.keyValueStore.set('userid', '123456');
    });
    app.get('/user', (req, res) {
      didHit = true;
      expect(req.keyValueStore.get<String>('userid'), '123456');
    });
...
List<HttpRequest> get _outstandingRequests => storeData.keys.toList();
