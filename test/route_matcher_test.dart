...
import 'package:alfred/src/http_route_match.dart';
...
void main() {
  ...
  tearDown(() {
    HttpRouteParam.paramParser.clear();
  });
  ...
  test('it should match routes correctly - custom typed parameters', () {
    ...
    HttpRouteParam.paramParser.add(frZip);
    HttpRouteParam.paramParser.add(frPhone);
    HttpRouteParam.paramParser.add(recentDate);
    HttpRouteParam.paramParser.add(refNumber);
    try {
      ...
      HttpRouteParam.paramParser.remove(refNumber);
      HttpRouteParam.paramParser.remove(recentDate);
      HttpRouteParam.paramParser.remove(frPhone);
      HttpRouteParam.paramParser.remove(frZip);
    }
  });
...
List<HttpRouteMatch> match(String input, List<HttpRoute> routes) =>
    RouteMatcher._foundMatch(input, routes, Method.get).toList();

List<HttpRoute> routes(Iterable<HttpRouteMatch> matches) =>
    matches.map((e) => e.methodRoute).toList();

List<String> patterns(Iterable<HttpRouteMatch> matches) =>
    matches.map((e) => e.methodRoute.route).toList();
...
class FrenchPostalCodeTypeParameter implements RouteSegmentParamType {
  @override
...
class FrenchPhoneNumberTypeParameter implements RouteSegmentParamType {
  @override
...
class RecentDateTypeParameter implements RouteSegmentParamType {
  @override
...
class RefNumberTypeParameter implements RouteSegmentParamType {
  @override
